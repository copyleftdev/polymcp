...
use super::types::ForexSnapshotResponse;

pub struct GetForexSnapshot {
    client: Arc<PolygonClient>,
}

impl GetForexSnapshot {
    ...
    async fn execute(&self, args: HashMap<String, Value>) -> Result<CallToolResult, PolygonError> {
        ...
        let url = match &tickers {
            Some(t) if !t.is_empty() => {
                format!(
                    "/v2/snapshot/locale/global/markets/forex/tickers?tickers={}",
                    t.join(",")
                )
            }
            _ => "/v2/snapshot/locale/global/markets/forex/tickers".to_string(),
        };

        let response: ForexSnapshotResponse = self.client.get(&url).await?;
...
impl Tool for GetForexSnapshot {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition::new("get_forex_snapshot")
            .with_description(
                "Get real-time snapshot data for forex currency pairs. \
                Returns current quotes, daily stats, and price changes.",
            )
            .with_schema(ToolInputSchema {
                ...
                properties: Some(HashMap::from([(
                    ...
                    json!({
                        "type": "array",
                        "items": { "type": "string" },
                        "description": "List of forex tickers (e.g., [\"C:EURUSD\", \"C:GBPUSD\"]). Omit for all pairs."
                    }),
                )])),
                required: None,
            })
