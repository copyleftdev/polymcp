...
use super::types::CryptoAggregatesResponse;

pub struct GetCryptoAggregates {
    client: Arc<PolygonClient>,
}

impl GetCryptoAggregates {
    ...
    async fn execute(&self, args: HashMap<String, Value>) -> Result<CallToolResult, PolygonError> {
        ...
        let response: CryptoAggregatesResponse = self.client.get(&url).await?;
...
impl Tool for GetCryptoAggregates {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition::new("get_crypto_aggregates")
            .with_description(
                "Get OHLCV aggregate bars for a cryptocurrency pair. \
                Use X: prefix for crypto tickers (e.g., X:BTCUSD for Bitcoin/USD).",
            )
