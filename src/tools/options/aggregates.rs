...
use super::types::OptionsAggregatesResponse;

pub struct GetOptionsAggregates {
    client: Arc<PolygonClient>,
}

impl GetOptionsAggregates {
    ...
    async fn execute(&self, args: HashMap<String, Value>) -> Result<CallToolResult, PolygonError> {
        let ticker = args
            .get("options_ticker")
            .and_then(|v| v.as_str())
            .ok_or_else(|| PolygonError::InvalidParams("options_ticker is required".to_string()))?;
        ...
        let response: OptionsAggregatesResponse = self.client.get(&url).await?;
...
impl Tool for GetOptionsAggregates {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition::new("get_options_aggregates")
            .with_description(
                "Get OHLCV aggregate bars for an options contract. \
                Requires the full options ticker (e.g., O:AAPL230120C00150000).",
            )
            .with_schema(ToolInputSchema {
                schema_type: "object".to_string(),
                properties: Some(HashMap::from([
                    (
                        "options_ticker".to_string(),
                        json!({
                            "type": "string",
                            "description": "Options contract ticker (e.g., O:AAPL230120C00150000)"
                        }),
                    ),
                    (
                        "from".to_string(),
